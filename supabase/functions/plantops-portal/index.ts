...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [estateRes, clientRes, orgRes] = await Promise.all([
      supabase.from('estates').select('id, name, address_text').eq('id', link.estate_id).maybeSingle(),
      link.client_id
        ? supabase.from('clients').select('id, name').eq('id', link.client_id).maybeSingle()
        : Promise.resolve({ data: null }),
      supabase.from('organizations').select('id, name').eq('id', link.org_id).maybeSingle(),
    ]);

    const payload: Record<string, unknown> = {
      estate: { name: estateRes.data?.name ?? null, address: estateRes.data?.address_text ?? null },
      client: (clientRes as any)?.data?.name ?? null,
      company: orgRes.data?.name ?? null,
      contact_note: link.contact_note ?? null,
      // The manual is always the internally approved snapshot, never auto-generated.
    ...
    if (link.show_plants) {
      const { data: placements } = await supabase
        ...
        .select('id, spot_label, next_water_due, last_watered_at, water_amount_note, client_instructions, do_not_do, care_responsibility, asset:assets!plant_placements_asset_id_fkey(name), zone:zones(name)')
    ...
    if (link.show_last_visit || link.show_history) {
      ...
      const { data: logs } = await supabase
        ...
        .select('id, action_type, performed_at, notes, placement_id')
        .eq('estate_id', link.estate_id)
