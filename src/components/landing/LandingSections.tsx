...
export function FinalCtaSection({ copy }: { copy: Copy['final'] }) {
  return <section className="final-cta"><div className="landing-shell"><HGTextReveal className="final-title">{copy.title}</HGTextReveal><HGReveal delay={120}><p>{copy.body}</p><div><Button asChild size="lg" className="bg-accent text-accent-foreground hover:bg-accent/90"><Link to="/auth?mode=signup">{copy.create}<ArrowRight /></Link></Button><Button asChild variant="outline" size="lg" className="border-primary-foreground/40 bg-transparent text-primary-foreground hover:bg-primary-foreground/10 hover:text-primary-foreground"><Link to="/auth">{copy.signIn}</Link></Button></div></HGReveal></div></section>;
}
